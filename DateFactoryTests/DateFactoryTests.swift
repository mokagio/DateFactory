...
class DateFactoryTests: XCTestCase {
  ...
  func testFactoryDefaultValue() {
    ...
    XCTAssertEqual(dateFormatter.stringFromDate(sut), "1970-01-01-00-00-00.000", "Expected date to be epoch")
  }
  ...
  func testFactoryWithValues() {
    ...
    XCTAssertEqual(dateFormatter.stringFromDate(sut), "2016-06-02-20-31-42.123", "Expected date to be built using the value passed to the method")
  }
  ...
  func testFactoryTimezone() {
    ...
    let timezone = NSTimeZone(forSecondsFromGMT: 3600)
    let sut = NSDate.date(
      year: 2016,
      month: 6,
      day: 2,
    ...
    XCTAssertEqual(dateFormatter.stringFromDate(sut), "2016-06-02-19-31-42.123", "Expected date to be built using the value passed to the method, and the given time zone")
  }
  ...
  func testFactoryTimezoneOnly() {
    ...
    // in a time zone y hours from GMT is (x - y) hours in the GMT time zone.
    let timezone = NSTimeZone(forSecondsFromGMT: 3600)
    let sut = NSDate.date(timeZone: timezone)

    XCTAssertEqual(dateFormatter.stringFromDate(sut), "1969-12-31-23-00-00.000", "Expected date to be built using the given time zone on top of epoch")
  }
}
